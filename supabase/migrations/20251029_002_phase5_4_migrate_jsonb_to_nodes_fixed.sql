-- ============================================================================
-- Migration: 20251029_002_phase5_4_migrate_jsonb_to_nodes (FIXED)
-- Description: Migrate existing JSONB nodes to mandala_nodes table
-- Dependencies: 20251029_001_phase5_4_mandala_nodes_table_fixed.sql
-- FIX: position → node_position (reserved keyword)
-- ============================================================================

-- ============================================================================
-- Data Migration Function (FIXED)
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      FOR node_record IN
        SELECT
          value->>'position' as position_str,
      ...
      LOOP
        -- mandala_nodes 테이블에 삽입 (FIXED: node_position 컬럼명 사용)
        INSERT INTO mandala_nodes (
          ...
          1, -- 기존 노드는 모두 level 1
          node_record.position_str::integer,
        ...
        RAISE NOTICE 'Migrated node: chart_id=%, node_id=%, position=%, title=%',
          chart_record.id, new_node_id, node_record.position_str, node_record.title;
      END LOOP;
...
-- ============================================================================
-- Verification Query (FIXED: node_position)
-- ============================================================================
...
-- 7. FIXED: position → node_position (reserved keyword 회피)
-- 8. NEXT STEP: 프론트엔드에서 mandala_nodes 테이블 사용하도록 API 함수 업데이트
-- ============================================================================
