-- ============================================================================
-- Migration: 20251029_001_phase5_4_mandala_nodes_table (FIXED)
-- Description: Phase 5.4 - Create mandala_nodes table for 81칸 expansion
-- Dependencies: mandala_charts, goals tables
-- FIX: position is a reserved keyword, use quoted identifiers
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS mandala_nodes (
  ...
  node_position INTEGER NOT NULL CHECK (node_position BETWEEN 1 AND 8),
  ...
  -- 제약 조건
  CONSTRAINT unique_node_position UNIQUE (mandala_chart_id, parent_node_id, level, node_position)
...
-- ============================================================================
-- Helper Function: Get Node Hierarchy (FIXED - quoted identifiers)
-- ============================================================================
...
-- ============================================================================
-- Helper Function: Get Child Nodes (FIXED)
-- ============================================================================
...
BEGIN
  ...
  ORDER BY n.node_position ASC;
END;
$$ LANGUAGE plpgsql;

-- ============================================================================
-- Helper Function: Get All Descendants (FIXED)
-- ============================================================================
...
BEGIN
  ...
  ORDER BY depth ASC, node_position ASC;
END;
$$ LANGUAGE plpgsql;
...
ALTER TABLE mandala_charts
ADD COLUMN IF NOT EXISTS max_level INTEGER DEFAULT 1 CHECK (max_level BETWEEN 1 AND 3);
...
COMMENT ON COLUMN mandala_nodes.node_position IS '1-8 시계방향 (1=상단)';
...
-- 6. FIXED: position → node_position (reserved keyword 회피)
-- 7. NEXT STEP: 기존 JSONB nodes 데이터를 mandala_nodes로 마이그레이션하는 스크립트 필요
-- ============================================================================
